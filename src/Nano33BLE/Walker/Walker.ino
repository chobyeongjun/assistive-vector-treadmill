 ...
 *     이유: 25Hz Teensy 맞춤 + partial packet BLE 전송 차단 (IMU 움직임 시 플롯 정지 버그 수정)
 * ================================================================
 */
...
// 링버퍼: 여러 패킷 수용 (패킷 ~70bytes × 6 = 420, 여유 마진)
...
void sendToBle() {
    ...
    // 40ms 타이머: 25Hz Teensy 전송 주기에 맞춤
    ...
    // 244B 이내의 완성 패킷만 전송해 BLE payload 중간 절단을 방지
    ...
    if (result) {
        // ★ 전송 후 남은 불완전 데이터를 버퍼 앞으로 이동
        dropBytes(sendLen);
        bleTxCount++;
    } else {
        // 실시간성 우선: 실패한 오래된 패킷은 버리고 다음 최신 패킷을 우선한다.
        bleWriteFailCount++;
        bleDropCount++;
        dropBytes(sendLen);
    }
