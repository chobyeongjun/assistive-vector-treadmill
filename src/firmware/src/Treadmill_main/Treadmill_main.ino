...
#define SPOOL_RADIUS_M 0.035f  // 풀리 직경 7cm → 반지름 3.5cm
 ...
 *  2) [ONSET, PEAK]: Rising Half-Sine  - 0N → 50N
...
inline void controllerStep(Side s, float dt) {
  ...
  if (currentMode == MODE_FORCE_ASSIST) {
    ...
    // ═══════════════════════════════════════════════════════════════
      ...
      F_cmd = 0.0f;
      inZone3 = true;  // 감기는 방향 전류 1A 제한
      M = adm_M_slack;
      C = adm_C_slack;
      max_vel = ZONE3_MAX_VEL_MPS;  // zero-force admittance 급반응 방지
      max_accel = MAX_ADM_ACCEL_SLACK;
    }
    // ═══════════════════════════════════════════════════════════════
    // ZONE 4: Post-Release (85%→100%) - Kinematic Payout FF
    // ★ hip extension 전환(RELEASE)부터 HS까지 능동 cable payout
    // ★ Zone 1 Motion FF 반전: -FF_GAIN_MOTION × ||v_imu||
    // ★ ZONE4_PAYOUT_DEG만큼 최대속도로 즉시 payout → 이후 zero current
    // ═══════════════════════════════════════════════════════════════
    else {
      // Zone 4 진입 시 시작 위치 기록 + admittance 상태 초기화
      if (wasInProfile[s]) {
        wasInProfile[s] = false;
        zone4EntryPos_deg[s] = motor_position_deg[s];
        adm_velocity_mps[s] = 0.0f;  // Zone 3 재진입 시 stale 값 방지
      }

      if (gcpActiveOf(s)) {
        float paidOut   = -(motor_position_deg[s] - zone4EntryPos_deg[s]);  // 양수 = 풀림
        float remaining = ZONE4_PAYOUT_DEG - paidOut;

        // 감속 시작 위치: v_max × RAMP_MS/2 를 도(deg)로 환산
        float decel_deg = rad_to_deg(MAX_ADM_VEL_FALLING * (ZONE4_RAMP_MS * 0.001f) * 0.5f / SPOOL_RADIUS_M);

        // Phase 1: 최대속도 (목표까지 감속 여유분보다 많이 남은 구간)
        // Phase 2: 위치 기반 linear ramp (감속 여유분 진입 후)
        float v_frac;
        if (remaining > decel_deg) {
          v_frac = 1.0f;
        } else if (remaining > 0.0f) {
          v_frac = remaining / decel_deg;
        } else {
          v_frac = 0.0f;
        }

        const float STOP_ERPM_THRESH = 500.0f;  // ~0.02 m/s, 정지 판정 임계값
        bool ramping = (v_frac > 0.0f);
        bool coasting = !ramping && (fabsf(motor_velocity_erpm[s]) > STOP_ERPM_THRESH);

        if (ramping) {
          // Phase 1/2: Hold → linear 감속
          float v_cmd = -MAX_ADM_VEL_FALLING * v_frac;
          float desired_erpm = clampf(mps_to_erpm(v_cmd), -MAX_VEL_ERPM, 0.0f);
          desiredCurrent_A[s] = pidOf(s).compute(desired_erpm, motor_velocity_erpm[s], dt);
          desiredVelocity_mps[s] = v_cmd;
          dbg_treadmill_ff_mps[s] = v_cmd;
          dbg_des_erpm[s] = desired_erpm;
        } else if (coasting) {
          // Phase 3: act vel > 0 → v_cmd=0으로 능동 제동, HS까지 대기
          desiredCurrent_A[s] = pidOf(s).compute(0.0f, motor_velocity_erpm[s], dt);
          desiredVelocity_mps[s] = 0.0f;
          dbg_treadmill_ff_mps[s] = 0.0f;
          dbg_des_erpm[s] = 0.0f;
        } else {
          // Phase 4: 완전 정지 → zero current, HS 감지까지 대기
          desiredCurrent_A[s] = 0.0f;
          desiredVelocity_mps[s] = 0.0f;
          dbg_treadmill_ff_mps[s] = 0.0f;
          dbg_des_erpm[s] = 0.0f;
        ...
        desiredForce_N[s] = 0.0f;
        dbg_F_assist[s] = 0.0f; dbg_F_cmd[s] = 0.0f; dbg_F_err[s] = 0.0f;
      } else {
        desiredForce_N[s] = 0.0f;
        desiredCurrent_A[s] = 0.0f;
        desiredVelocity_mps[s] = 0.0f;
        adm_velocity_mps[s] = 0.0f;
        dbg_F_assist[s] = 0.0f; dbg_F_cmd[s] = 0.0f;
        dbg_F_err[s] = 0.0f; dbg_des_erpm[s] = 0.0f;
      }
      return;
    }
    ...
    dbg_treadmill_ff_mps[s] = v_treadmill_ff;
      ...
      // Zone 3: 감기는 방향(양수) 전류 제한 → 장력 과도 buildup 방지
      if (inZone3) {
        desiredCurrent_A[s] = fminf(desiredCurrent_A[s], ZONE3_MAX_WIND_CURRENT_A);
      }
...
void handleCommand(String cmd) {
  ...
  // ★ z4: Zone 4 payout 각도 [deg] (기본 100)
  if (cmd.startsWith("z4")) {
    noInterrupts();
    ZONE4_PAYOUT_DEG = clampf(cmd.substring(2).toFloat(), 0.0f, 360.0f);
    interrupts();
    Serial.println("ZONE4_PAYOUT_DEG = " + String(ZONE4_PAYOUT_DEG, 1));
    return;
  }
  // ★ zr: Zone 4 감속 ramp 시간 [ms] → decel_deg = v_max×T/2 결정 (기본 40ms)
  if (cmd.startsWith("zr")) {
    noInterrupts();
    ZONE4_RAMP_MS = clampf(cmd.substring(2).toFloat(), 10.0f, 500.0f);
    interrupts();
    Serial.println("ZONE4_RAMP_MS = " + String(ZONE4_RAMP_MS, 1));
    return;
  }
  ...
  // ★ z3c: Zone 3 감기는 방향 전류 제한 [A] (기본 1A)
  if (cmd.startsWith("z3c")) {
    noInterrupts();
    ZONE3_MAX_WIND_CURRENT_A = clampf(cmd.substring(3).toFloat(), 0.0f, MAX_CURRENT_A);
    interrupts();
    Serial.print("ZONE3_MAX_WIND_CURRENT = "); Serial.println(ZONE3_MAX_WIND_CURRENT_A, 2);
    return;
  }
  // ★ z3v: Zone 3 zero-force admittance 속도 제한 [m/s] (기본 0.5)
  if (cmd.startsWith("z3v")) {
    noInterrupts();
    ZONE3_MAX_VEL_MPS = clampf(cmd.substring(3).toFloat(), 0.0f, MAX_ADM_VELOCITY_MPS);
    interrupts();
    Serial.print("ZONE3_MAX_VEL = "); Serial.println(ZONE3_MAX_VEL_MPS, 2);
    return;
  }
